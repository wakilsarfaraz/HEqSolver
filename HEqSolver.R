...
for (n in 1:NumTRI){
  ...
  for (i in 1:3){
    for (j in 1:3){
      SP_Stiff[LocNodes[n,i],LocNodes[n,j]]=SP_Stiff[LocNodes[n,i],LocNodes[n,j]]+Astiff[i,j]
      SP_Mass[LocNodes[n,i],LocNodes[n,j]]=SP_Mass[LocNodes[n,i],LocNodes[n,j]]+Amass[i,j]
    }
  }
}
